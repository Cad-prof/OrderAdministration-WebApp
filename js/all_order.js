...
const url = "http://127.1.2.3:3000/selectadmin"; // Spécifiez le port 3000 et l'adresse de votre API
...
Http.onreadystatechange = function () {
  if (this.readyState == 4 && this.status == 200) {
    ...
    for (var item = 0; item < json.length; item++) {
      ...
      var date =
        isoDate.slice(3, 6) + isoDate.slice(0, 3) + isoDate.slice(6, 10);
      ...
      if (json[item].order_status === "New order") {
        ...
        btn.setAttribute(
          "onclick",
          "goToConfirm(" + json[item].order_id + ")"
        );
